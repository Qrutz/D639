...
class AngleCalculator {
    public:
        AngleCalculator();
        float CalculateSteeringAngle(cv::Mat &yellowInputImage, cv::Mat &blueInputImage, float &steeringWheelAngle, bool isClockwise, float maxSteering, float minSteering);

    private:
        cv::Point calculateCentroid(const std::vector<std::vector<cv::Point>>& contours, const cv::Point& imageCenter);
        float adjustSteering(const cv::Point& centroid, const cv::Point& center, bool shouldSteerRight);
